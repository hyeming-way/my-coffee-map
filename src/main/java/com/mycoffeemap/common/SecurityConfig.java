...
import com.mycoffeemap.user.CustomOAuth2UserService;
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    	...
    	http.authorizeHttpRequests( auth ->
    								...
    								//loginPage(): 인증되지 않은 사용자가 보호된 자원에 접근 시 리디렉션할 로그인 페이지 URL 설정
										.defaultSuccessUrl("/mycoffeemap"))
    									//defaultSuccessUrl(): 로그인 성공 후 이동할 기본 경로 설정
    								//소셜 로그인
    								.oauth2Login( oauth2 -> oauth2.loginPage("/user/login")
    							          .userInfoEndpoint( userInfo -> userInfo.userService(customOAuth2UserService) )
    							          .defaultSuccessUrl("/mycoffeemap", true) )
